...
class Query(graphene.ObjectType):
    travelers = graphene.List(TravelerType, userId = graphene.Int())
    travelersConcat = graphene.List(TravelerListType, userId = graphene.Int())
    ...
    def resolve_travelersConcat(self, info, **kwargs):
        userId = kwargs.get('userId')
        if userId is not None:
            try:
                User.objects.using('default').get(user_id=userId)
