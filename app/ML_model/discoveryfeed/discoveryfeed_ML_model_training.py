...
user_df = pd.DataFrame(user_records, columns = column_names)
...
user_df['age'] = (now - user_df['DOB']).astype('<m8[Y]')    # 3
# Dropping DOB column
user_df = user_df.drop(['DOB'], axis=1)
# Filling missing value for language_preference column

user_df["language_preference"]=user_df["language_preference"].fillna('English')
...
(case when evo.exp_venue_option_name='Individual' then 1 else 0 end ) as Individual,
(case when evo.exp_venue_option_name='Couple' then 1 else 0 end ) as Couple,
(case when evo.exp_venue_option_name='Group' then 1 else 0 end ) as Groupss
...
left join public.exp_venue_option evo
on vi.venue_internal_id =evo.venue_internal_id 
group by p.post_id,
p.title,
p.user_rating,
p.is_verified_booking,
...
column_names_post = ['post_id','venue_id','title','user_rating','is_verified_booking','post_description',
                    ...
                    'report_post_count','post_saved_count','individual','couple','groups']
...
post_df['age'] = (now - post_df['DOB']).astype('<m8[Y]')    # 3

post_df = post_df.drop(['individual','couple','groups'], axis=1)

# handling missing values
post_df["language_preference"]=post_df["language_preference"].fillna('English')
post_df["video_duration"]=post_df["video_duration"].fillna(0)
post_df["watch_time"]=post_df["watch_time"].fillna(0)
post_df["venue_level_id"]=post_df["venue_level_id"].fillna(0)
post_df["languages_id"]=post_df["languages_id"].fillna(0)
...
post_df = post_df.astype({'venue_type_id':np.int64,'languages_id':np.int64})

df_merged =  user_df.merge(post_df, how = 'cross')

# Removing rows where auth user and post author is same
df_final = df_merged[df_merged['user_id_x'] != df_merged['user_id_y']]
...
# adding interactions to the df_final    
...
# functions to be run before running the scripts
